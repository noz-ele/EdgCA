...
- **発行 layer は RSA / Ed25519 / 非 NIST curve なし**。発行 API (`createRootCA` / `issueIntermediateCA` / `issueClientCert` / `issueDocumentSigningCert`) と CSR API は ECDSA NIST P-256 / P-384 / P-521 (それぞれ SHA-256 / SHA-384 / SHA-512 の標準ペアリング) のみ受け、それ以外は発行/CSR 境界で reject する。**`exportPkcs12` は別**: algorithm 非依存の PKCS#12 packer であり、任意の PKCS#8 DER bytes (RSA / Ed25519 / 任意 curve の ECDSA など) を受ける。PKCS#12 wrapping は byte-level (PBES2 で暗号化して埋め込むだけ) で、内部 algorithm を見る必要が無いため。
