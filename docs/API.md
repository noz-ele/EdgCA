...
root CA または intermediate CA から intermediate CA を発行します。
...
`pathLenConstraint` 省略時は `0` です。

issuer CA が `pathLenConstraint=0` の場合、この関数は例外を投げます。これは、leaf certificate だけを発行できる CA の下にさらに intermediate CA を作らないためです。
...
- `pathLenConstraint=0` の CA から intermediate CA を発行しようとした。
